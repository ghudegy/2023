...
이 문제는 원래 UCPC에 출제하려고 계획했던 문제였습니다. 지문에 그 흔적이 남아 있습니다.
...
서브태스크 2와 3을 풀기 위해서는 다음의 중요한 성질을 파악해야 합니다.
...
따라서 $A$를 흰색 모자가 $w_1$개 이상 $w_2$개 이하인 모자 조합의 집합으로 초기화하고, 각 질문마다 $i$번째 출제자가 보고 있는 정보를 만족하는 조합의 집합 $X_i$와 $A$의 교집합의 모든 원소에 대해 $i$번째 출제자의 모자의 색깔이 같은지를 판정하고, 이를 바탕으로 $A$라는 집합을 업데이트하면 문제를 풀 수 있습니다. 이 때 $A$의 각 원소마다 대답이 어떻게 나올지를 모두 알아야 $A$를 업데이트할 수 있습니다. 이를 구현하면 $\mathcal{O}(QN \cdot 4^N)$ 정도의 시간복잡도로 문제를 해결할 수 있습니다.

### 서브태스크 3

$A$의 각 원소마다, $i$번째 출제자의 대답이 어떻게 나올지를 모두 판단하는 것을 $\mathcal{O}(2^N)$에 할 수 있습니다. $i$번째 출제자가 보고 있는 정보가 같은 원소들에 대해서는 답이 똑같기 때문입니다. 따라서 각 쿼리를 $\mathcal{O}(N \cdot 2^N)$에 처리할 수 있습니다. 최적화 여부에 따라 실행 시간이 유의미하게 차이가 나는데, $\mathcal{O}(QN \cdot 2^N)$의 시간복잡도로 푼 코드들이 대부분 정답을 받을 수 있도록 시간 제한을 10초로 여유롭게 설정하였습니다.
...
이 방식으로 구현된 출제자의 코드는 1 722 바이트입니다. 코드가 엄청나게 기괴하지는 않습니다.
...
PS를 조금만 해 보셨다면 이들 코드는 합해 봐야 천 바이트도 안 나올 것 같다는 생각이 드실 것입니다. 실제로 PS 헤더를 안 쓰기만 해도 무난하게 통과할 수 있습니다. 코드를 길게 짜기로 유명한 출제자가 코드 제한을 전혀 신경쓰지 않고 짜서 1 902 바이트가 나왔습니다.
